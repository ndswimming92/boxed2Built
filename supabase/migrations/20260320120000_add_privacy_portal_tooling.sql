/*
  ...
  ## Adds
  - Customer privacy requests (data export + account deletion)
  - Async privacy export jobs
  - Marketing consent preferences
  - Immutable privacy audit events
  - Customer/admin RPCs for workflow operations
*/
...
CREATE TRIGGER trg_customer_privacy_export_jobs_updated_at
  BEFORE UPDATE ON public.customer_privacy_export_jobs
  FOR EACH ROW
  EXECUTE FUNCTION public.set_privacy_portal_updated_at();
...
CREATE TRIGGER trg_customer_marketing_consents_updated_at
  BEFORE UPDATE ON public.customer_marketing_consents
  FOR EACH ROW
  EXECUTE FUNCTION public.set_privacy_portal_updated_at();
...
AS $$
BEGIN
  RAISE EXCEPTION 'privacy_audit_events is immutable';
END;
$$;
...
CREATE TRIGGER trg_privacy_audit_events_no_update
  BEFORE UPDATE ON public.privacy_audit_events
  FOR EACH ROW
...
CREATE TRIGGER trg_privacy_audit_events_no_delete
  BEFORE DELETE ON public.privacy_audit_events
  FOR EACH ROW
  EXECUTE FUNCTION public.prevent_privacy_audit_mutation();
...
BEGIN
  ...
  SELECT
    ...
    'data_export',
    'pending',
    jsonb_build_object('channel', 'portal_privacy')
  FROM public.customers c
  WHERE c.id = v_customer_id
  RETURNING * INTO v_request;

  INSERT INTO public.customer_privacy_export_jobs (
    request_id,
    customer_id,
    organization_id,
    status
...
BEGIN
  ...
  PERFORM public.log_privacy_event(
    ...
    'customer_privacy_requests',
    v_request.id,
    jsonb_build_object('status', v_request.status, 'grace_period_ends_at', v_request.grace_period_ends_at)
  );
...
BEGIN
  ...
  SET
    status = 'scheduled',
    confirmed_at = now(),
    metadata = r.metadata || jsonb_build_object('confirmed_via', 'portal')
  WHERE r.id = p_request_id
    AND r.customer_id = v_customer_id
    AND r.request_type = 'account_deletion'
    AND r.status = 'pending_confirmation'
    AND r.confirmation_token = p_confirmation_token
  RETURNING r.* INTO v_request;

  IF v_request.id IS NULL THEN
    RAISE EXCEPTION 'Invalid request confirmation details.';
  END IF;
  ...
  PERFORM public.log_privacy_event(
    ...
    'customer_privacy_requests',
    v_request.id,
    jsonb_build_object('confirmed_at', v_request.confirmed_at)
  );
...
CREATE OR REPLACE FUNCTION public.upsert_my_marketing_consent(
  ...
  p_sms_marketing_enabled boolean,
  p_consent_source text DEFAULT 'portal_privacy'
)
RETURNS public.customer_marketing_consents
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
...
BEGIN
  ...
  INSERT INTO public.customer_marketing_consents (
    ...
    COALESCE(p_sms_marketing_enabled, false),
  ...
  DO UPDATE SET
    ...
    consented_at = CASE
      WHEN EXCLUDED.email_marketing_enabled OR EXCLUDED.sms_marketing_enabled THEN now()
      ELSE public.customer_marketing_consents.consented_at
    END,
    revoked_at = CASE
      WHEN NOT EXCLUDED.email_marketing_enabled AND NOT EXCLUDED.sms_marketing_enabled THEN now()
      ELSE NULL
    END
  ...
  PERFORM public.log_privacy_event(
    ...
    jsonb_build_object(
      ...
      'sms_marketing_enabled', v_consent.sms_marketing_enabled,
...
CREATE OR REPLACE FUNCTION public.admin_update_privacy_export_job(
  p_job_id uuid,
  p_status text,
...
RETURNS public.customer_privacy_export_jobs
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
...
BEGIN
  ...
  SET
    status = p_status,
    download_url = COALESCE(p_download_url, j.download_url),
    expires_at = COALESCE(p_expires_at, j.expires_at),
  ...
  WHERE j.id = p_job_id
    AND public.can_manage_org_settings(j.organization_id)
  ...
  SELECT request_id INTO v_request_id FROM public.customer_privacy_export_jobs WHERE id = p_job_id;
...
BEGIN
  ...
  WHERE r.id = p_request_id
    AND r.request_type = 'account_deletion'
    AND public.can_manage_org_settings(r.organization_id)
    AND (
      p_status <> 'completed'
      OR (
        r.confirmed_at IS NOT NULL
        AND r.grace_period_ends_at IS NOT NULL
        AND now() >= r.grace_period_ends_at
      )
    )
  RETURNING r.* INTO v_request;

  IF v_request.id IS NULL THEN
    RAISE EXCEPTION 'Deletion request not found, not eligible, or access denied.';
  END IF;
  ...
  PERFORM public.log_privacy_event(
    ...
    v_request.customer_id,
    'admin',
    'deletion_request_updated',
    'customer_privacy_requests',
    v_request.id,
    jsonb_build_object('status', v_request.status, 'processed_at', v_request.processed_at)
  );
...
CREATE POLICY "Customer can view own export jobs"
  ON public.customer_privacy_export_jobs
  FOR SELECT TO authenticated
  USING (customer_id = public.current_customer_id());
...
CREATE POLICY "Customer can view own marketing consents"
  ON public.customer_marketing_consents
  FOR SELECT TO authenticated
  USING (customer_id = public.current_customer_id());
...
CREATE POLICY "Customer and admins can view privacy audit events"
  ...
  USING (
    ...
    OR public.can_view_org_data(organization_id)
  );
