/*
  # Fix client metrics calculation to use correct column name

  ## Overview
  Updates the update_client_metrics function to use job_status instead of status
  column name for jobs table queries.

  ## Changes
  - Updates all references from status to job_status
*/
...
BEGIN
  ...
  -- Count completed jobs (using job_status not status)
  SELECT COUNT(*)
  INTO v_job_count
  FROM jobs
  WHERE client_id = client_id_input
