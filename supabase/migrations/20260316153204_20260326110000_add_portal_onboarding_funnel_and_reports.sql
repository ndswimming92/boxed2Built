...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'customers' AND column_name = 'invited_at'
  ) THEN
    ALTER TABLE public.customers ADD COLUMN invited_at timestamptz;
  END IF;
END $$;
...
CREATE POLICY "Customers can view own funnel events"
  ON public.portal_funnel_events FOR SELECT TO authenticated
...
CREATE POLICY "Customers can insert own funnel events"
  ON public.portal_funnel_events FOR INSERT TO authenticated
...
CREATE POLICY "Org members can manage welcome queue"
  ON public.portal_welcome_email_queue FOR ALL TO authenticated
  USING (public.is_organization_member(organization_id))
  WITH CHECK (public.is_organization_member(organization_id));

CREATE OR REPLACE FUNCTION public.enqueue_portal_welcome_sequence_for_customer(p_customer_id uuid)
RETURNS void
...
BEGIN
  ...
  IF v_customer.id IS NULL OR NULLIF(BTRIM(v_customer.email), '') IS NULL THEN RETURN; END IF;

  INSERT INTO public.portal_welcome_email_queue (organization_id, customer_id, sequence_step, template_key, scheduled_for, metadata)
  VALUES
...
CREATE OR REPLACE FUNCTION public.track_portal_funnel_event(p_event_type text, p_metadata jsonb DEFAULT '{}'::jsonb)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_customer_id uuid;
...
BEGIN
  ...
  SELECT organization_id INTO v_organization_id FROM public.customers WHERE id = v_customer_id;
  ...
  IF p_event_type IN ('first_job_view', 'walkthrough_completed') THEN
    INSERT INTO public.portal_funnel_events (organization_id, customer_id, event_type, metadata)
    VALUES (v_organization_id, v_customer_id, p_event_type, COALESCE(p_metadata, '{}'::jsonb))
    ON CONFLICT (customer_id, event_type) WHERE event_type IN ('first_job_view', 'walkthrough_completed')
    DO NOTHING;
    RETURN;
  END IF;

  SELECT COUNT(*)::integer INTO v_login_count FROM public.portal_funnel_events
  WHERE customer_id = v_customer_id AND event_type = 'login';
