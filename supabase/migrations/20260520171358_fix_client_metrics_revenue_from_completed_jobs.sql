/*
  # Fix client metrics to calculate revenue from completed jobs

  ## Summary
  The `update_client_metrics` function was incorrectly calculating `total_revenue`
  from paid invoices, which left `average_job_value` at $0 for clients who have
  completed jobs but no paid invoices. This migration fixes revenue to come from
  completed jobs using `final_price` (falling back to `quoted_price`).

  ## Changes
  1. `update_client_metrics` function - revenue source changed from paid invoices
     to SUM(final_price) on completed jobs
  2. Backfills all existing clients so stored values are corrected immediately

  ## Important Notes
  - Revenue now reflects completed work regardless of invoice payment status
  - `average_job_value` = total_revenue / job_count (for completed jobs)
  - Free jobs (is_free = true) are excluded from revenue calculations
*/
...
BEGIN
  ...
  -- Revenue from completed paid jobs (excludes free/pro-bono jobs)
  ...
  -- Count all completed jobs (including free)
  SELECT COUNT(*)
  INTO v_job_count
  FROM jobs
  WHERE client_id = client_id_input
...
-- Backfill all clients so stored metrics are corrected immediately.
DO $$
DECLARE
  r record;
BEGIN
