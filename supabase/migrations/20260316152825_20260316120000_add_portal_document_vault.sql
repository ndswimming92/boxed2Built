...
BEGIN
  SELECT r.retention_days INTO v_retention_days
...
CREATE POLICY "Org admins can manage document retention rules"
  ON public.document_retention_rules FOR ALL TO authenticated
...
CREATE POLICY "Customers can view owned portal documents"
  ...
  USING (owner_customer_id = public.current_customer_id() AND is_visible_to_customer = true AND deleted_at IS NULL);
...
CREATE POLICY "Customers can view own document audit events"
  ON public.portal_document_audit_events FOR SELECT TO authenticated
...
CREATE POLICY "Customers can insert own document audit events"
  ON public.portal_document_audit_events FOR INSERT TO authenticated
...
BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Org members can manage portal vault objects"
      ...
      USING (bucket_id = 'portal-document-vault' AND public.is_platform_admin())
      WITH CHECK (bucket_id = 'portal-document-vault' AND public.is_platform_admin());
  END IF;
END $$;
...
BEGIN
  ...
  INSERT INTO public.portal_document_audit_events (
    organization_id, customer_id, document_id, event_type, ip_address, user_agent
  ) VALUES (
    v_doc.organization_id, v_customer_id, v_doc.id,
