/*
  ...
  - Primary key: `(organization_id, lower(email))`
  - Organization context precedence:
    1) row.organization_id
    2) business_info.organization_id via row.business_id
  - If no normalized email or no organization context, row is queued for manual review.

  ## Migration notes
  - This migration is additive and non-destructive: no existing columns are removed.
  - Existing `client_id` relationships remain intact while `customer_id` is introduced.
  - Backfill is idempotent and only populates rows where `customer_id` is NULL.

  ## Rollback plan (manual)
  - Delete or archive dependent application code using `customer_id`.
  - Drop FKs/indexes on `customer_id` columns from touched tables.
  - Drop `customer_identity_review_queue` and `customers` tables.
  - Optional: set all populated `customer_id` values to NULL before dropping columns.
*/

-- 1) Core secure ownership table
...
-- Identity constraints/indexes used for lookup
...
-- Reuse existing org/view helper functions for baseline access
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 2) Manual review queue for unmatched or ambiguous records
...
-- 3) Add customer_id FK columns to customer-facing/history tables
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 4) Backfill customers + table customer_id values
WITH source_rows AS (
  SELECT
    c.organization_id,
...
-- Backfill customer_id in jobs
UPDATE public.jobs j
SET customer_id = c.id
FROM public.customers c
LEFT JOIN public.business_info bi ON bi.id = j.business_id
WHERE j.customer_id IS NULL
  AND c.organization_id = COALESCE(j.organization_id, bi.organization_id)
  AND LOWER(c.email) = LOWER(j.client_email)
  AND NULLIF(BTRIM(j.client_email), '') IS NOT NULL;

-- Backfill customer_id in invoices
UPDATE public.invoices i
SET customer_id = c.id
FROM public.customers c
LEFT JOIN public.business_info bi ON bi.id = i.business_id
WHERE i.customer_id IS NULL
  AND c.organization_id = COALESCE(i.organization_id, bi.organization_id)
  AND LOWER(c.email) = LOWER(i.client_email)
  AND NULLIF(BTRIM(i.client_email), '') IS NOT NULL;

-- Backfill customer_id in form_inquiries
UPDATE public.form_inquiries fi
SET customer_id = c.id
FROM public.customers c
LEFT JOIN public.business_info bi ON bi.id = fi.business_id
WHERE fi.customer_id IS NULL
  AND c.organization_id = COALESCE(fi.organization_id, bi.organization_id)
  AND LOWER(c.email) = LOWER(fi.client_email)
  AND NULLIF(BTRIM(fi.client_email), '') IS NOT NULL;

-- Backfill customer_id in saved_requests
UPDATE public.saved_requests sr
SET customer_id = c.id
FROM public.customers c
LEFT JOIN public.business_info bi ON bi.id = sr.business_id
WHERE sr.customer_id IS NULL
  AND c.organization_id = COALESCE(sr.organization_id, bi.organization_id)
  AND LOWER(c.email) = LOWER(sr.client_email)
  AND NULLIF(BTRIM(sr.client_email), '') IS NOT NULL;

-- Backfill customer_id in job_completions via jobs
UPDATE public.job_completions jc
SET customer_id = COALESCE(j.customer_id, c.id)
FROM public.jobs j
LEFT JOIN public.customers c
  ON c.organization_id = COALESCE(j.organization_id, (SELECT organization_id FROM public.business_info WHERE id = j.business_id))
 AND LOWER(c.email) = LOWER(j.client_email)
WHERE jc.customer_id IS NULL
  AND jc.job_id = j.id;


-- 5) Manual review queue population for unmatched/ambiguous records
-- Unmatched jobs
...
SELECT
  ...
  jsonb_build_object(
    ...
    'furniture_type', fi.furniture_type
  )
FROM public.form_inquiries fi
LEFT JOIN public.business_info bi ON bi.id = fi.business_id
WHERE fi.customer_id IS NULL
...
-- Optional ambiguity queue: row has email but no org context and multiple customers share that email across orgs
INSERT INTO public.customer_identity_review_queue (
  source_table, source_pk, organization_id, business_id, candidate_email, reason, payload
)
SELECT
  ...
  j.business_id,
  j.client_email,
  'ambiguous_multi_org_email',
  jsonb_build_object('client_name', j.client_name)
FROM public.jobs j
WHERE j.customer_id IS NULL
  AND NULLIF(BTRIM(j.client_email), '') IS NOT NULL
  ...
  AND (
    SELECT COUNT(*)
    FROM public.customers c
    WHERE LOWER(c.email) = LOWER(j.client_email)
  ) > 1
ON CONFLICT (source_table, source_pk) DO NOTHING;
