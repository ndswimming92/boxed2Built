...
BEGIN
  ...
  INSERT INTO public.support_tickets (
    organization_id, customer_id, related_job_id, related_invoice_id,
    subject, priority, sla_first_response_due_at, sla_resolution_due_at, last_customer_message_at
  ) VALUES (
    ...
    v_customer_id, p_related_job_id, p_related_invoice_id,
    btrim(p_subject), lower(coalesce(p_priority, 'normal')),
    now() + interval '4 hours', now() + interval '2 days', now()
  ) RETURNING * INTO v_ticket;

  INSERT INTO public.support_ticket_messages (
    organization_id, ticket_id, customer_id, author_user_id, author_role, message_body
  ) VALUES (
    v_ticket.organization_id, v_ticket.id, v_ticket.customer_id, auth.uid(), 'customer', btrim(p_message)
  );
...
CREATE OR REPLACE FUNCTION public.add_support_ticket_message(p_ticket_id uuid, p_message text)
...
BEGIN
  ...
  SET status = CASE WHEN status = 'resolved' THEN 'waiting_on_customer' ELSE status END,
      last_customer_message_at = now(), updated_at = now()
...
CREATE OR REPLACE FUNCTION public.admin_add_support_ticket_message(p_ticket_id uuid, p_message text, p_is_internal boolean DEFAULT false)
RETURNS public.support_ticket_messages
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  INSERT INTO public.support_ticket_messages (
    organization_id, ticket_id, customer_id, author_user_id, author_role, message_body, is_internal
  ) VALUES (
    v_ticket.organization_id, v_ticket.id, v_ticket.customer_id, auth.uid(), 'admin', btrim(p_message), coalesce(p_is_internal, false)
  ) RETURNING * INTO v_message;
...
CREATE OR REPLACE FUNCTION public.admin_update_support_ticket_status(p_ticket_id uuid, p_status text, p_note text DEFAULT NULL, p_priority text DEFAULT NULL)
RETURNS public.support_tickets
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF NOT public.is_organization_member(v_ticket.organization_id) THEN RAISE EXCEPTION 'Not authorized to update this support ticket'; END IF;
  IF v_status NOT IN ('open', 'in_progress', 'waiting_on_customer', 'resolved', 'closed') THEN RAISE EXCEPTION 'Invalid status'; END IF;
  IF p_priority IS NOT NULL AND v_priority NOT IN ('low', 'normal', 'high', 'urgent') THEN RAISE EXCEPTION 'Invalid priority'; END IF;
  ...
  INSERT INTO public.support_ticket_messages (
    organization_id, ticket_id, customer_id, author_user_id, author_role, message_body, is_internal, metadata
  ) VALUES (
    v_ticket.organization_id, v_ticket.id, v_ticket.customer_id, auth.uid(), 'system',
    COALESCE(NULLIF(btrim(p_note), ''), format('Ticket status updated to %s', replace(v_status, '_', ' '))),
    false, jsonb_build_object('event_type', 'status_change', 'status', v_status)
  );
