/*
  # Fix update_client_metrics to use correct job_status column name

  ## Summary
  The previous version of update_client_metrics referenced `status` on the jobs table,
  but the actual column name is `job_status`. This migration corrects that and also
  fixes the completion_date reference (jobs uses `completion_date` which is correct).

  ## Changes
  - Updated function: `update_client_metrics`
    - Changed `status = 'completed'` to `job_status = 'completed'` in both job queries
*/
