/*
  ...
  1) Adds a customer-scoped request table for job action intents:
     - request_reschedule
     - cancel_request
     - add_note
  2) Enforces server-side constraints via SECURITY DEFINER RPC functions.
  3) Adds moderation endpoint for org admins with explicit pending/approved/rejected state.
  4) Emits customer notifications for moderation outcomes.
  5) Writes immutable audit rows for every submitted action and moderation decision.
*/
...
CREATE TABLE IF NOT EXISTS public.job_customer_action_requests (
  ...
  CONSTRAINT check_reschedule_date_required
    CHECK (
      ...
      OR requested_schedule_date IS NOT NULL
    )
);
...
CREATE POLICY "Customer can view own job action requests"
  ON public.job_customer_action_requests
  FOR SELECT
  TO authenticated
...
CREATE POLICY "Customer can view own job action request audit events"
  ON public.job_customer_action_request_audit_events
  FOR SELECT
  TO authenticated
  USING (customer_id = public.current_customer_id());
...
BEGIN
  ...
  IF p_action_type = 'request_reschedule' THEN
    ...
    IF v_job.date_scheduled IS NULL THEN
      RAISE EXCEPTION 'Job has no scheduled date to reschedule';
    END IF;

    IF v_job.date_scheduled <= now() + interval '24 hours' THEN
      RAISE EXCEPTION 'Reschedule requests require at least 24 hours notice';
    END IF;
    ...
    IF p_requested_schedule_date < now() + interval '24 hours' THEN
      RAISE EXCEPTION 'Requested schedule date must be at least 24 hours from now';
    END IF;

    IF p_requested_schedule_date > now() + interval '60 days' THEN
      RAISE EXCEPTION 'Requested schedule date must be within 60 days';
    END IF;
  END IF;
  ...
  IF p_action_type = 'cancel_request' THEN
    ...
    IF v_job.date_scheduled IS NOT NULL AND v_job.date_scheduled <= now() + interval '12 hours' THEN
      RAISE EXCEPTION 'Cancel requests require at least 12 hours notice before the scheduled start';
    END IF;
  END IF;
  ...
  INSERT INTO public.job_customer_action_requests (
    ...
    v_job.id,
    p_action_type,
    NULLIF(btrim(p_request_message), ''),
    p_requested_schedule_date,
    'pending'
  )
  ...
  INSERT INTO public.job_customer_action_request_audit_events (
    ...
    jsonb_build_object(
      ...
      'requested_schedule_date', v_request.requested_schedule_date,
  ...
  INSERT INTO public.admin_audit_logs (
    ...
    auth.uid(),
    COALESCE((auth.jwt() ->> 'email'), 'customer-portal@system.local'),
    'CREATE',
    'job_customer_action_requests',
    v_request.id,
    v_request.action_type,
    to_jsonb(v_request),
    'Customer submitted a job action request from portal',
    'success',
    jsonb_build_object('job_id', v_request.job_id, 'customer_id', v_request.customer_id, 'notify_admin', true)
  );
...
BEGIN
  SELECT *
  INTO v_request
  FROM public.job_customer_action_requests r
  ...
  INSERT INTO public.admin_audit_logs (
    ...
    CASE WHEN v_decision = 'approved' THEN 'APPROVE' ELSE 'REJECT' END,
    'job_customer_action_requests',
    v_request.id,
    v_request.action_type,
    NULL,
    to_jsonb(v_request),
    format('Admin %s customer job action request', v_decision),
    'success',
    jsonb_build_object('job_id', v_request.job_id, 'customer_id', v_request.customer_id)
  );
  ...
  PERFORM public.enqueue_customer_notification(
    ...
    jsonb_build_object(
      ...
      'action_type', v_request.action_type,
      'moderation_note', v_request.moderation_note
    ),
    true
  );
