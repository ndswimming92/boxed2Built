/*
  # Client Merge Function
  
  Creates an RPC function that merges two client records into one.
  All related records (jobs, invoices, inquiries, notes) are reassigned 
  from the discarded client to the kept client. Missing contact info on
  the kept client is backfilled from the discarded one. A system note
  documents the merge.
*/

CREATE OR REPLACE FUNCTION public.merge_clients(
  p_keep_client_id uuid,
  p_discard_client_id uuid
)
...
BEGIN
  ...
  INSERT INTO public.admin_audit_logs (organization_id, action_type, table_name, record_id, record_identifier, metadata)
  VALUES (
    v_keep.organization_id,
    'MERGE',
    'clients',
    p_keep_client_id::text,
    v_keep.name,
    jsonb_build_object(
