/*
  ...
  ## Summary
  1. Adds `customer_notifications` store with read state, payload, and timestamps.
  2. Adds optional email mirror queue + per-customer unsubscribe preferences.
  3. Produces notifications from key lifecycle events:
     - job scheduled / completed
     - invoice issued / paid
     - reminder sent
  4. Enforces strict customer-scoped RLS for portal access.
*/
  ...
  IF (TG_OP = 'INSERT' AND NEW.job_status = 'scheduled')
    ...
    PERFORM public.enqueue_customer_notification(
      ...
      jsonb_build_object(
        ...
        'date_scheduled', NEW.date_scheduled,
...
DROP POLICY IF EXISTS "Org admins can view notification email queue" ON public.customer_notification_email_queue;
CREATE POLICY "Org admins can view notification email queue"
  ON public.customer_notification_email_queue
  FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1
      FROM public.customers c
      WHERE c.id = customer_notification_email_queue.customer_id
        AND public.can_manage_org_settings(c.organization_id)
    )
  );
