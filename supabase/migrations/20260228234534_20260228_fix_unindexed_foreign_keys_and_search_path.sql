/*
  # Fix Unindexed Foreign Keys, Unused Index, and Mutable Search Path

  ## Summary
  Addresses all reported security/performance advisories:
  1. Adds covering indexes for every unindexed foreign key column across 30+ tables
  2. Drops the unused index `idx_client_notes_created_by`
  3. Locks the search_path on `update_job_status_timestamp` to prevent search-path injection

  ## New Indexes (grouped by table)
  ...
  ### customer_reviews
  - job_completion_id, organization_id
  ...
  - completed_by, created_by, job_completion_id, job_id, organization_id

  ### job_completions
  - completed_by, job_id, organization_id

  ### jobs
  - completion_id, organization_id

  ### mileage_records
  - business_id, expense_id, organization_id
  ...
  ## Dropped Indexes
  - `idx_client_notes_created_by` — unused

  ## Function Fix
  - `update_job_status_timestamp` — SET search_path = pg_catalog, public
*/
...
-- Fix mutable search_path on update_job_status_timestamp
