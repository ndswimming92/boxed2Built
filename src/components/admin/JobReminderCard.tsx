...
import {
  ...
  getJobReminderPreview,
  sendJobReminderNow,
  sendJobReminderTest,
  type JobReminderPreview,
} from '../../services/jobReminderService';

interface JobReminderCardProps {
  jobId: string;
  /** Shown in the send confirmation so nobody mails the wrong customer. */
  clientName: string;
}

/**
 * The customer's day-before reminder, as it will arrive, with the one line that
 * says whether it is going out.
 *
 * The body is rendered by the edge function that sends it, so what shows here
 * is the email itself rather than a second template maintained in the browser.
 * Nothing here is editable on purpose: when the arrival time or the address is
 * wrong, the job is wrong — fix the job and the preview follows. That keeps one
 * source of truth between what the schedule says and what the customer reads.
 *
 * Mounted only inside an expanded job card or an open client profile, so a page
 * of collapsed rows costs nothing — each mount is an edge function call.
 */
export default function JobReminderCard({ jobId, clientName }: JobReminderCardProps) {
  const [preview, setPreview] = useState<JobReminderPreview | null>(null);
  ...
  useEffect(() => {
    ...
    getJobReminderPreview(jobId)
      .then((result) => {
        if (!cancelled) setPreview(result);
      })
      .catch((err: unknown) => {
        if (!cancelled) setError(err instanceof Error ? err.message : 'Could not load the reminder');
      })
      .finally(() => {
    ...
    async (mode: 'send' | 'test') => {
      if (mode === 'send') {
        ...
        const warning = preview?.status === 'sent'
          ? `${clientName} has already been reminded. Send this email again to ${to}?`
          : `Send this reminder to ${to} now, ahead of its scheduled time?`;
      ...
      try {
        const result = mode === 'send'
          ? await sendJobReminderNow(jobId)
          : await sendJobReminderTest(jobId);
        setNotice(
          mode === 'send'
  ...
  // A job with no date has no reminder to describe, and the job card already
  // says it is unscheduled.
          ...
          {preview.html && (
            <>
              <div className="flex flex-wrap items-center gap-2 mt-3">
                ...
                {/* Only offered when the email could actually go out. A blocked
                    job would refuse server-side anyway; not showing the button
              ...
              {expanded && (
                <div className="mt-3">
                  ...
                  {/* Sandboxed with no allow-* flags: this is our own markup, but
                      an email body is never worth granting scripts or same-origin. */}
                  <iframe
                    title="Customer reminder email preview"
                    srcDoc={preview.html}
                    sandbox=""
