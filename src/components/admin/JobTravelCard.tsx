...
import { getDirectionsUrl, resolveWorkAddress } from '../../utils/jobAddress';
import {
  formatTravelDistance,
  formatTravelDuration,
  getJobTravelEstimate,
  type JobTravelEstimate,
} from '../../services/jobTravelService';

interface JobTravelCardProps {
  job: Job;
}
 ...
 *
 * Mounted only when a job card is expanded, so a page of collapsed jobs costs
 * nothing. The map arrives as a data URI because the Mapbox call happens in the
 * `job-travel-estimate` edge function — an <img> tag cannot send an
 * Authorization header, so a URL the browser could load directly would have to
 * be public.
 */
export default function JobTravelCard({ job }: JobTravelCardProps) {
  ...
  useEffect(() => {
    ...
    // A job that collapses mid-flight must not write into an unmounted card.
    return () => {
      cancelled = true;
    };
  }, [job.id, workAddress, attempt]);
