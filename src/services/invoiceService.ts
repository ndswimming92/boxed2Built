...
export async function calculateLateFee(invoiceId: string): Promise<number> {
  ...
  if (error) {
    ...
    throw new Error(`Failed to calculate late fee: ${error.message}`);
  }

  return data || 0;
}

export async function applyLateFees(): Promise<{ invoice_id: string; late_fee_applied: number }[]> {
  const { data, error } = await supabase.rpc('apply_late_fees_to_overdue_invoices');
...
export async function manuallyAdjustLateFee(
  invoiceId: string,
  lateFeeAmount: number,
  reason?: string
): Promise<void> {
  const { error } = await supabase.rpc('manually_adjust_late_fee', {
    p_invoice_id: invoiceId,
  ...
  if (error) {
    console.error('Error adjusting late fee:', error);
    throw new Error(`Failed to adjust late fee: ${error.message}`);
  }
}
